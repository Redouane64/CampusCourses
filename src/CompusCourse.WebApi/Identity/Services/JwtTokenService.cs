...
using CompusCourse.WebApi.Identity.Extensions;
...
namespace CompusCourse.WebApi.Identity.Services
{

    public sealed class JwtTokenService<TUser> where TUser: class
    {
        ...
        public async Task<(string Token, string RefreshToken)> RefreshToken(string refreshToken, TUser user, IEnumerable<Claim> claims)
        {
            ...
            if (!isValid)
            {
                throw new Error(IdentityErrorCodes.InvalidRefreshToken, 401);
            }
